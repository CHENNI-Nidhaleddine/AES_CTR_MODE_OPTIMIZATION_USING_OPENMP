...
void increment_ctr(BYTE ctr[], int counter_size, int step)
{
...
void aes_encrypt_ctr(const BYTE in[], size_t in_len, BYTE out[], const WORD key[], int keysize, const BYTE* ctr)
{
    size_t idx = 0, last_block_length;
    BYTE ctr_buf[AES_BLOCK_SIZE], out_buf[AES_BLOCK_SIZE];

    if (in != out)
        memcpy(out, in, in_len);

    memcpy(ctr_buf, &ctr[0], AES_BLOCK_SIZE);

    //extract the last bloc to use it outside if (3lajal cas ou last bloc size <16bytes)
    ...
    if (in_len > AES_BLOCK_SIZE)
    {
        for (idx = 0; idx < last_block_length; idx += AES_BLOCK_SIZE)
        {
            ...
            xor_buffer(out_buf, &out[idx], AES_BLOCK_SIZE); //xor avec data block
            memcpy(ctr_buf, &ctr[idx + AES_BLOCK_SIZE], AES_BLOCK_SIZE);
        }
    }
    // dernier bloc de donnees
    encrypt(ctr_buf, out_buf, key, keysize);
    xor_buffer(out_buf, &out[idx], in_len - idx); // Use the Most Significant bytes.
}

void aes_encrypt_ctr_openmp(const BYTE in[], size_t in_len, BYTE out[], const WORD key[], int keysize, const BYTE ctr[])
{

    //calculer le nombre de blocs total
    ...
    //copying the input text to output that will contain the result of encryption (or decryption)
    if (in != out)
        memcpy(out, in, in_len);
  
    last_block_length = in_len % AES_BLOCK_SIZE == 0 ? 0 : in_len - ((in_len / AES_BLOCK_SIZE) * AES_BLOCK_SIZE);
    int NB_THREADS = num_blocks >= NUM_THREADS ? NUM_THREADS : num_blocks;
    #pragma omp parallel  num_threads(NB_THREADS)
        {
            int idx;
            BYTE ctr_buf[AES_BLOCK_SIZE], out_buf[AES_BLOCK_SIZE];
            int th_idx = omp_get_thread_num();
            memcpy(ctr_buf, ctr, AES_BLOCK_SIZE);
            int blocks_per_thread = num_blocks / omp_get_num_threads();
            
            int rest_of_blocks = num_blocks-NB_THREADS * blocks_per_thread;
            int from = th_idx * blocks_per_thread;
            int to = th_idx == NB_THREADS - 1 ? (th_idx + 1) * blocks_per_thread + rest_of_blocks : (th_idx + 1) * blocks_per_thread;
            int sizeofLastBlock = in_len % AES_BLOCK_SIZE == 0 ? 0 : in_len - ((in_len / AES_BLOCK_SIZE) * AES_BLOCK_SIZE);
            increment_ctr(ctr_buf, 16, th_idx * blocks_per_thread);
            for (idx = from; idx < to-1 ; idx++)
            {
              
                encrypt(ctr_buf, out_buf, key, keysize); //cryptage du ctr
                xor_buffer(out_buf, &out[idx*AES_BLOCK_SIZE], 16); //xor avec data block
            
                increment_ctr(ctr_buf, AES_BLOCK_SIZE, 1);//
              
            }
            encrypt(ctr_buf, out_buf, key, keysize); //cryptage du ctr
            xor_buffer(out_buf, &out[idx*AES_BLOCK_SIZE], sizeofLastBlock); //xor avec data block
        }  
}
...
//
//void aes_encrypt_ctr_cuda(const BYTE in[], size_t in_len, BYTE out[], const WORD key[], int keysize, const BYTE ctr[])
//{
//    size_t idx = 0, last_block_length;
//    BYTE ctr_buf[AES_BLOCK_SIZE], out_buf[AES_BLOCK_SIZE];
//
...
//        {
//            encrypt(ctr_buf, out_buf, key, keysize); //cryptage du ctr
//            xor_buffer(out_buf, &out[idx * AES_BLOCK_SIZE], AES_BLOCK_SIZE); //xor avec data block
//            increment_ctr(ctr_buf, AES_BLOCK_SIZE);//
//        }
//    }
//
//    // dernier bloc de donnees
//    encrypt(ctr_buf, out_buf, key, keysize);
//    xor_buffer(out_buf, &out[idx * AES_BLOCK_SIZE], last_block_length); // Use the Most Significant bytes.
//}
//void aes_encrypt_ctr2(const BYTE in[], size_t in_len, BYTE out[], const WORD key[], int keysize, const BYTE ctr[])
//{
//    size_t idx = 0, last_block_length;
//    BYTE ctr_buf[AES_BLOCK_SIZE], out_buf[AES_BLOCK_SIZE];
//
